...
BOOL CViewDlg::OnInitDialog() 
{
   ...
   if (numPages > 1)
   {
      ...
      m_pView->GetPageFormat(pageFormat);
...
{
   // Update display page according to selected view format
   int index= m_cbViewFormat.GetCurSel();
   if (index != CB_ERR)
      m_pView->SetPage(index);
}
